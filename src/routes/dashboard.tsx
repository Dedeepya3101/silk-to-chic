...
import { Upload, Sparkles, MessageCircle, Heart, Bell, CheckCircle2, MapPin, Star, ImageIcon, ArrowRight } from "lucide-react";
...
export const Route = createFileRoute("/dashboard")({
  head: () => ({ meta: [{ title: "Dashboard — MatchO" }] }),
  component: UserDashboard,
});

function UserDashboard() {
  return (
    <AppShell role="user" title="Hi Aanya, ready to reimagine?">
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-gradient-primary p-6 text-primary-foreground shadow-elegant">
      ...
      <div className="mt-5 grid grid-cols-4 gap-2 text-[10px]">
        {["Style", "Occasion", "Fabric", "Color", "Budget", "Notes"].slice(0, 4).map(t => (
  ...
  return (
    <div className="flex items-center gap-4 rounded-2xl bg-card p-3 shadow-soft transition hover:-translate-y-0.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 text-xs text-muted-foreground flex items-center gap-2"><MapPin className="h-3 w-3" /> {distance} · <Star className="h-3 w-3 fill-gold text-gold" /> {rating}</p>
      </div>
