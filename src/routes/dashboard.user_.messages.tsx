...
function UserMessages() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (s.role !== "user") { navigate({ to: "/dashboard/tailor/messages", replace: true }); return; }
      setMe(s.id);
  ...
  const load = async (uid: string) => {
    ...
    const merged = list.map(t => {
      ...
      return { ...t, tailor_name: p?.display_name || "Tailor", avatar_url: p?.avatar_url, image_url: um.get(t.saree_upload_id) };
    });
    ...
    <AppShell role="user" title="Messages">
      {loading ? (
        ...
        <div className="grid place-items-center rounded-3xl glass p-12 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">When a tailor sends you a suggestion, your conversation will appear here.</p>
        </div>
      ) : (
        <div className="grid gap-5 lg:grid-cols-[320px_1fr]">
          <aside className="glass rounded-3xl p-3 shadow-soft">
            <p className="px-2 py-2 text-xs uppercase tracking-widest text-muted-foreground">Conversations</p>
                  ...
                  <li key={t.id}>
                    ...
                    >
                      {t.avatar_url ? (
                        ...
                        <div className="grid h-10 w-10 place-items-center rounded-full bg-foreground text-background text-sm">{t.tailor_name?.[0]}</div>
                      )}
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-medium">{t.tailor_name}</p>
                        <p className={`truncate text-xs ${activeId === t.id ? "opacity-80" : "text-muted-foreground"}`}>{last?.message || t.silhouette || "Suggestion sent"}</p>
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        <span className={`text-[10px] ${activeId === t.id ? "opacity-80" : "text-muted-foreground"}`}>{fmtTime(new Date(lastTs).toISOString())}</span>
          ...
          <section className="glass flex min-h-[60vh] flex-col rounded-3xl shadow-soft">
            {active ? (
              <>
                ...
                <div className="flex gap-2 border-t border-border p-4">
                  ...
                  <button
                    ...
                    disabled={sending}
                    className="inline-flex items-center gap-1 rounded-full bg-gradient-primary px-4 py-2 text-sm text-primary-foreground disabled:opacity-60"
                  >
